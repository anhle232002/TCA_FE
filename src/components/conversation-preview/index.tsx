import React from "react";
import { Link } from "react-router-dom";

interface Props {
    header: string;
    subheader: string;
    link: string;
    image: string;
}

export const ConversationPreview: React.FC<Props> = ({ header, subheader, link, image }) => {
    return (
        <Link to={link}>
            <div
                role="button"
                className="flex gap-4 px-4 border-b dark:border-b-dark__accent-content last:border-none py-5 hover:bg-secondary-content dark:hover:bg-dark__secondary-content duration-100 group"
            >
                <div className="flex-shrink-0">
                    <img
                        className="rounded-full w-10 h-10"
                        src={
                            image ||
                            ` 
                        data:image/jpeg;base64,`
                        }
                        alt=""
                    />
                </div>
                <div className="max-w-[180px] flex-1">
                    <h3 className="text-base">{header}</h3>
                    <p className="opacity-80 text-xs tracking-wide mt-1  whitespace-nowrap overflow-hidden overflow-ellipsis">
                        {subheader}
                    </p>
                </div>

                <div className="group-hover:opacity-100 opacity-0  text-primary text-xl flex-shrink-0">
                    <span className="">
                        <i className="ri-more-fill"></i>
                    </span>
                </div>
            </div>
        </Link>
    );
};
